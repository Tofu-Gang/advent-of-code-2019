...
def puzzle_01() -> int:
    ...
    :return: Answer should be 402879.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        lines = tuple(map(str.strip, f.readlines()))
        orbits_map = CosmicMap(lines)
        result = orbits_map.orbit_count_checksum
...
def puzzle_02() -> int:
    ...
    :return: Answer should be 484.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        lines = tuple(map(str.strip, f.readlines()))
        orbits_map = CosmicMap(lines)
        result = orbits_map.santa_transfers_count
        print_puzzle_solution(result)
        return result
