...
PHASE_SETTINGS_FEEDBACK_LOOP = permutations([5, 6, 7, 8, 9], 5)
...
def puzzle_01() -> int:
    ...
    :return: Answer should be 14902.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        program = f.read().strip()
        max_thrusters_signal = -maxsize - 1

        for phase_settings in PHASE_SETTINGS:
            amplifiers = Amplifiers(program, tuple(phase_settings))
            amplifiers.configure()
            max_thrusters_signal = max(
                max_thrusters_signal, amplifiers.thrusters_signal)

        result = max_thrusters_signal
...
def puzzle_02() -> int:
    ...
    :return: Answer should be 6489132.
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        for phase_settings in PHASE_SETTINGS_FEEDBACK_LOOP:
            amplifiers = Amplifiers(program, tuple(phase_settings))
            amplifiers.configure()
            max_thrusters_signal = max(
                max_thrusters_signal, amplifiers.thrusters_signal)
